...
- **Wordlist picker** — for directory/file brute-forcing tools (`ffuf`, `gobuster`), a "Select wordlist" button opens a card-based picker with two tabs, plus a "tool default" option. Picking a wordlist swaps just the `-w <path>` argument in the command, leaving everything else untouched. Hidden for tools that don't take a wordlist.
  ...
  - **SecLists (GitHub)** — browse every wordlist in [danielmiessler/SecLists](https://github.com/danielmiessler/SecLists) without cloning the ~1GB repo. Picking a file downloads *only that one file* (via GitHub's API + raw file CDN) into `~/.surveil/wordlists/seclists/`, mirroring the repo's own folder structure, and selects it immediately. A file downloaded this way then also shows up under the **Local** tab from then on (see `surveil/seclists_remote.py`). The category listing itself is cached for 24h to stay well under GitHub's unauthenticated API rate limit.
